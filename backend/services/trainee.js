    ...
    if(errors){
       ...
       // var location = req.body.location;
        ...
        TestPaperModel.findOne({ _id : testid, isRegistrationavailable : true }).then((info)=>{
            if(info){
                TraineeEnterModel.findOne({$or:[{userid : userid , testid : testid}]}).then((data)=>{
                    if(data){
                        console.log(data);
                        res.json({
                            success : false,
                        ...
                        tempdata.save().then((u)=>{
                                sendmail(userid,"Registered Successfully",`You have been successfully registered for the test. Click on the link given to take test  "${req.protocol + '://' + req.get('host')}/trainee/taketest?testid=${testid}&traineeid=${u._id}"`).then((dd)=>{
                                    console.log(dd)
                                }).catch((errr)=>{
                                    console.log(errr);
                                })
...
let correctAnswers = (req,res,next)=>{
    ...
    TestPaperModel.find({_id:_id,testconducted:true},{type:0,subjects:0,duration:0,difficulty:0,testbegins:0,status:0,createdBy:0,isRegistrationavailable:0,testconducted:0})
...
let resendmail = (req,res,next)=>{
    ...
    TraineeEnterModel.findById(userid,{userid:1,testid : 1}).then((info)=>{
        if(info){
            console.log(info)
            sendmail(info.userid,"Registered Successfully",`You have been successfully registered for the test. Click on the link given to take test  "${req.protocol + '://' + req.get('host')}/trainee/taketest?testid=${info.testid}&traineeid=${info._id}"`).then((dd)=>{
                console.log(dd)
            }).catch((errr)=>{
                console.log(errr);
            })
...
let Testquestions = (req,res,next)=>{
    ...
    TestPaperModel.findById(testid,{type:0,title:0,subjects:0,difficulty:0,testbegins:0,status:0,createdBy:0,isRegistrationavailable:0})
   .populate('questions','body')
   .populate({ 
     path: 'questions',
     model: QuestionModel,
...
let TraineeDetails = (req,res,next)=>{
    ...
    TraineeEnterModel.findById(traineeid,{name:1,userid:1}).then((info)=>{
        if(info){
            res.json({
                success : true,
...
let getAllTrainees = (req,res,next)=>{
    TraineeEnterModel.find().then((info)=>{
        console.log(info)

        if(info){
            const key = 'userid';
            const unique = [...new Map(info.map(item => [item[key], item])).values()]
            res.json({
                success : true,
                message : 'Trainee details',
                data : unique
...
module.exports = {traineeenter,feedback,checkFeedback,resendmail,correctAnswers,Answersheet,flags,chosenOptions,TraineeDetails,Testquestions,UpdateAnswers,EndTest,getQuestion, getAllTrainees}