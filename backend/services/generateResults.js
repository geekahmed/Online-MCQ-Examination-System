                ...
                if(!results){
                    ...
                    .exec(function(err,answersheet){
                        if(err){
                            ...
                            if(!answersheet){
                                reject(new Error("مدخلات غير صحيحة"))
                            }
                            else{
                                var Score = 0;
...
let generateResultsForUser = (req, res, next) => {
    var userid = req.params.userid;
    ResultModel.find({}, {result:0, answerSheetid:0, _id:0, __v:0})
    .populate({
        path: 'userid',
        match: {
            userid: {
    ...
    .exec((err, res1) => {
        let fi = res1.filter(elem => {
            return elem.userid !== null;
        })
        res.json({
            success : true,
            message : `Success`,
...
/*     ResultModel.find({userid: userid}, {result:0, answerSheetid:0, _id:0, __v:0})
        .populate('testid', 'title-_id')
        .populate('userid', 'userid-_id')
        .exec((err, res1) => {
            console.log(res1);
            res.json({
                success : true,
...
module.exports = {generateResults,generateResultsForUser,gresult}