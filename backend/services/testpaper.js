...
let basicTestdetails = (req,res,next)=>{
 ...
 let getTestquestions = (req,res,next)=>{
     if(req.user.type==="TRAINER"){
         ...
         TestPaperModel.findById(testid,{type:0,title:0,subjects:0,duration:0,difficulty:0,testbegins:0,status:0,createdBy:0,isRegistrationavailable:0})
        .populate('questions','body')
        .populate({ 
          path: 'questions',
          model: QuestionModel,
 ...
 let beginTest = (req,res,next)=>{
    if(req.user.type==="TRAINER"){
        var id = req.body.id;
        TestPaperModel.findOneAndUpdate({_id:id,testconducted : false},{testbegins:1,isRegistrationavailable:1},{new: true})
        .then((data)=>{
            if(data){
                res.json({
                    success : true,
 ...
 let endTest = (req,res,next)=>{
    if(req.user.type==="TRAINER"){
        var id = req.body.id;
        TestPaperModel.findOneAndUpdate({_id:id,testconducted:0,testbegins:1,isResultgenerated:0},{testbegins:false,testconducted:true, isResultgenerated:true, isRegistrationavailable: false},{
            new: true
          })
        .then((info)=>{
            if(info){
