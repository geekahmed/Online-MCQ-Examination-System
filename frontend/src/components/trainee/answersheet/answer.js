    ...
    componentDidMount(){
        ...
        Promise.all([p1,p2,p3]).then(d=>{
            ...
            if(d[0].data.success && d[1].data.success){
                ...
                Alert('error','خطأ!',`${d[0].data.success ? "":d[0].data.message} and ${d[1].data.success ? "":d[1].data.message}`)
            }
        }).catch((err)=>{
            console.log(err)
            this.setState({
    ...
    render(){
        ...
        return (
            <div className="answer-table-outer">
                <Title className="answer-table-heading" level={1}>النتيجة</Title>
                <div className="answer-table-wrapper">
                    <Descriptions bordered title={null} border size="small">
                        <Descriptions.Item label="الإسم">{td.name}</Descriptions.Item>
                        <Descriptions.Item label="معرف المستخدم">{td.userid}</Descriptions.Item>
                        <Descriptions.Item label="النتيجة">{this.state.TotalScore}</Descriptions.Item>
                    </Descriptions>
                    <br/>
                    <Table 
                        size="small"
