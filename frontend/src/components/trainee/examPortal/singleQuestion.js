    ...
    onAnswerChange=(d1,d2,d3)=>{
        ...
        if(d2){
            if(this.state.ticked===ansCount){
                return Alert('error','خطأ!','امسح الخيارات المحددة لتحديد خيار آخر')
            }
            else{
                    ...
                    <Row type="flex">
                        ...
                        <Col span={22} order={2}>
                            <Radio.Group  style={{float:'right'}}>
                                <Radio.Button style={{background:'#009999',color:'#fff'}}>{this.props.trainee.questions[this.props.trainee.activeQuestionIndex].anscount===1?"نوع إجابة واحدة":"نوع إجابة متعددة"}</Radio.Button>
                                <Radio.Button style={{background:'#009999',color:'#fff'}}>علامات : {this.props.trainee.questions[this.props.trainee.activeQuestionIndex].weightage}</Radio.Button>
                            </Radio.Group>
                        </Col>
                    </Row>
                    {
                    ...
                    <div className="Question-single-option-panel">
                        <Row type='flex'>
                            {this.state.options.map((d,i)=>{
                                return(
                                    <Col span={12} key={i} className="Single-option" order={1}>
                                        <Row type='flex'>
                                            <Col span={2} style={{textAlign:'center'}} order={1}>
                                            ...
                                            <Col span={22} style={{padding:'10px'}} order={2}>
                ...
                <div className="control-button-in-exam-portal">
                    <Button.Group>
                        ...
                        <Button type="default" onClick={this.mark}>
                                <Icon type="flag" />
                                {!this.props.trainee.answers[this.props.trainee.activeQuestionIndex].isMarked?"تحديد السؤال":"قم بإلغاء تحديد السؤال"}
                        </Button>
                        {this.props.trainee.activeQuestionIndex===this.props.trainee.questions.length-1?
                            ...
                            <Button style={{background:'#009999',color:'#fff'}} onClick={this.next}>
                                {this.state.AnswerSelected?"حفظ & التالي": "التالي"}
                                <Icon type="right" />
                            </Button>
                        }
