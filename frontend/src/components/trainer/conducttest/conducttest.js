        ...
        if(this.state.needRedirect){
            ...
            return (
                <div className="conduct-test-main-wrapper">
                    ...
                    <div>
                        <Title style={{width:'100%',textAlign:'center'}} level={4}>أدخل معرف الاختبار</Title>
                        <div className="test-conduct-testid-form">
                            <Input placeholder="أدخل معرف الاختبار" onChange={this.ChangeLocalTestId} value={this.state.localTestId}/>
                            <Button onClick={this.ChangetestId}  type="primary" style={{marginTop:'30px'}}>تابع</Button>
                        </div>
                    </div>:
                    <div>
        ...
        if(this.props.conduct.basictestdetails.testconducted){
            return(
                <div className="reasendmail-container-register">
                    <Title style={{color:'#fff'}} level={4}>انتهى الاختبار! اذهب إلى جميع الاختبارات للتحقق من النتائج</Title>    
                </div>
            )
        }
        else{
            return (
                <div>
                    <TestDetails/>
                    <Tabs defaultActiveKey="1" style={{marginTop:'20px'}} type="card">
                        <TabPane tab={<span><Icon type="user" />متدرب مسجل</span>} key="1">
                            <Candidates />
                        </TabPane>
                        <TabPane tab={<span><Icon type="question-circle" />الأسئلة</span>} key="2">
